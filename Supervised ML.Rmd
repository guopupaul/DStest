...
##Remove outliers
```{r}
a <- boxscores.active %>% select( c("TP", "RESULT"))
w <- a %>% filter(RESULT == 1)
l <- a %>% filter(RESULT == 0)
summary(w)
...
boxscores.active <- boxscores %>% select(-c(RND, TEAM, "3P%", "2P%", "FT%", MIN)) %>%
...
auc(ROC)
```
The high AUC score for the logistic regression model (0.9855) suggests the model performs well i.e. it ranks a random positive example more highly than a random negative example.
...
```
 ...
 The confusion matrix also provides information about the precision and recall(proportion of actual wins identified correctly) of the model.
  The accuracy rate is (24/28) = 0.85714. When the model predicts a win, it is correct 86% of the time. 
 A model that always identifies actual wins has a recall of 1.0. Here (60/68) = 0.88235. The model correctly identifies wins 88% of the time.
...
mean(fit - actual)
```
By subtracting the average of actual wins from the average of predicted wins, the result is -0.02941176, which  confirms no material prediction bias in the model.
##Feature selection
