...
##Method
1. Conduct a logistic regression for a response variable of "WIN/LOSS" as a function of all dependent variables
2. Reduce the number of dependent variables to avoid overfitting and validate a final model which would be easier to use
3. Identify 5-6 key perfomance targets
...
auc(ROC)
```
The high AUC score for the logistic regression model (0.9825) suggests the model performs well i.e. it ranks a random positive example more highly than a random negative example.
...
fit<- boxscores.active.test$win_pred
actual <- boxscores.active.test$RESULT
as.matrix(table(actual, fit))
mean(boxscores.active.test$win_pred == boxscores.active.test$RESULT)
```
 The mean equivalence between predicted wins and actual wins in the test data (0.8676471) suggests the model's accuracy will make 87 correct predictions out of 100 total examples.

 The confusion matrix also provides information about the precision (false positive rate) and recall(proportion of actual wins identified correctly) of the model.
 A model that produces no false positives has a precision of 1.0. Here (29/29+4) = 0.8787879. When the model predicts a win, it is correct 88% of the time. 
 A model that always identifies actual wins has a recall of 1.0. Here (29/29+5) = 0.8529412. The model correctly identifies wins 85% of the time.
...
```{r}
mean(boxscores.active.test$win_pred) - mean(boxscores.active.test$RESULT)
```
By subtracting the average predicted wins from the average of actual wins, the result is -0.01470588, which is effectively zero. This confirms no prediction bias in the model.
