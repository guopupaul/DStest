...
glimpse(boxscores)
predictors <- boxscores %>% select(3:22) %>% select(-c("3P%","2P%","FT%"))
predictors <- sapply(predictors, as.numeric)
predictors <- as.data.frame(predictors)
```

PCA
```{r}
res.pca <- PCA(predictors, graph = FALSE)
...
In this case, 7 dimensions > 1 and cover 82% of the variance.
...
```
7 dimensions still looks right.
