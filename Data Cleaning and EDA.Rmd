...
```{r}
# x is male data, y is female data
tidyjoin <- function(x,y)  {
x <- x %>% mutate(gender = "Male")
y <- y %>% mutate(gender = "Female")
#join the two tibbles
bothsex <-bind_rows(x,y)
return(bothsex)
}
```
A tidy data tibble showing male prisoner age across 2007-2018:
```{r}
tidy_male_prisoner_age <- tidy_up(male_prisoner_age)
tidy_male_prisoner_age
```
A tidy data tibble showing female prisoner age across 2007-2018:
```{r}
tidy_female_prisoner_age <- tidy_up(female_prisoner_age)
