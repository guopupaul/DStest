...
```
The code below is a function that tidies extracted data:
```{r}
...
```
The code below joins male and female data
```{r}
# x is male data, z is female data
tidyjoin <- function(x,z)  {
  x <- x %>% mutate(gender = "Male")
  z <- z %>% mutate(gender = "Female")
  #join the two tibbles
  bothsex <-bind_rows(x,z)
  bothsex <- bothsex %>% 
    mutate_all(list(~str_replace(.,"Under 20", "0-20" ))) %>% 
    mutate_all(list(~str_replace(.,"60 and over", "60-150" ))) 
  bothsex <- bothsex %>% arrange(year)
  bothsex <- bothsex[, c(2,1,4,3)]
  bothsex$year <- dmy(bothsex$year)
  bothsex$number_of_prisoners <- as.numeric(bothsex$number_of_prisoners)
  return(bothsex)
}
```
...
The code below joins male and female tibbles together:
```{r}
tidy_prisoner_rate <- tidyjoin(tidy_male_prisoner_rate, tidy_female_prisoner_rate)
...
```{r}
tidy_male_prisoner_prior <- tidy_up(male_prisoner_prior) %>%
rename( "Priors" = "Age")
tidy_male_prisoner_prior
```
A tidy data tibble for female priors
```{r}
tidy_female_prisoner_prior <- tidy_up(female_prisoner_prior) %>%
rename( "Priors" = "Age")
...
```{r}
tidy_male_prisoner_offence <- tidy_up(male_prisoner_offence) %>%
rename( "Offence" = "Age")
tidy_male_prisoner_offence
```
A tidy data tibble for female offences
```{r}
tidy_female_prisoner_offence <- tidy_up(female_prisoner_offence) %>%
rename( "Offence" = "Age")
...
```{r}
prisoner_birth <- read_excel("annual_prisonerstats_2017-18v2.xls", sheet = 15, range = "B6:AA44",col_names = FALSE)
```
a tidy data tibble
```{r}
tidy_prisoner_birth <- tidy_up(prisoner_birth)
tidy_prisoner_birth
```
